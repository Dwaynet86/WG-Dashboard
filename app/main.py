...
async def add_client(request: Request,
    ...
    if ip is None: # if no ip enetered use next available
       proc = subprocess.run(["pivpn", "-a", "-n", client_name, "-ip", "auto"], capture_output=True, text=True)
    else: # if ip enetered use it
       proc = subprocess.run(["pivpn", "-a", "-n", client_name, "-ip", ip], capture_output=True, text=True) 
